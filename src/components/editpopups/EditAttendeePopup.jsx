...
function EditAttendeePopup({setPopup, eventId, attendeeInfo, sucessCallback}) {
    ...
    async function handleAddAttendee(e) {
        e.preventDefault();
        const result = await fetch("/api/updateattendee", {
            method : "POST",
            headers : {
        ...
        if(result.ok) {
            ...
            if(res.success) {
                console.log(res);
                toast.success('Attendee Updated Sucessfully');
                sucessCallback(attendee.inviteid);
                setPopup(false);
            }
            else {
